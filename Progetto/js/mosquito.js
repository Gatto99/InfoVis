var delayTime = 200; // quanto aspettare per eseguire la transizione
...
function drawMosquito(firstDataCase){
    ...
    svg.append("svg")
        .attr("id", "mosquitoSVG") // svg contenente la zanzara, così si evita che si debba cliccare esattamente sul path
        .append("path")            // path della zanzara
        .attr("id", "mosquito")
...
function updateMosquitoPosition(data, idxNextDataCase){
    ...
    let numDataCase = 10; // in questo caso lo sappiamo
    ...
    // @TODO: animazione deve essere smooth
    // spostamento del path della zanzara
    d3.select("#mosquito").transition().ease(d3.easeQuad).duration(delayTime).attr("transform", "translate(" + xMosquito + "," + yMosquito + ")")
...
d3.json("dataset/dataset.json")
    .then(function(data){
        ...
        var mosquito = d3.select("#mosquitoSVG"); // prendo l'svg contente la zanzara
